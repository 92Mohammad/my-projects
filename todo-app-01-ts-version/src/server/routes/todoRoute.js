...
router.get('/getAllTodo', function (req, res) {
    ...
    var sql = 'SELECT todo_id, task FROM todos WHERE userId = ?';
    connectDB_1.default.query(sql, [userId], function (err, results) {
        if (err) {
...
router.post('/createNewTodo', function (req, res) {
    var task = req.body.task;
    var loggedUserId = req.userId;
    if (!task || !loggedUserId) {
        return res.status(401).send({ message: "Please fulfill the input box" });
    }
    var values = [task, loggedUserId];
    var sql = 'INSERT INTO todos (task, userId) VALUES(?, ?)';
    connectDB_1.default.query(sql, values, function (err, results) {
        if (err) {
...
router.post('/updateTodo', function (req, res) {
    var _a = req.body, todoId = _a.todoId, title = _a.title;
    console.log(todoId, title);
    var sql = 'UPDATE todos SET task = ? WHERE todo_id = ?';
    connectDB_1.default.query(sql, [title, todoId], function (err, results) { return __awaiter(void 0, void 0, void 0, function () {
        return __generator(this, function (_a) {
            if (err) {
                ...
                return [2 /*return*/, res.status(200).json({ message: "todo updated successfully" })];
            }
            return [2 /*return*/];
        });
    }); });
...
router.post('/deleteTodo', function (req, res) {
    ...
    var sql = 'DELETE FROM todos WHERE todo_id = ?';
    connectDB_1.default.query(sql, todoId, function (err, results) {
        if (err) {
